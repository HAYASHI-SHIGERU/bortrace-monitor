...
GitHub Actionsを利用して、ボートレース場での「波乱レース（1号艇が1番人気でないレース）」を24時間監視し、Discordに通知するツールです。
...
3.  **Check**:
    ...
    *   対象レースの **単勝オッズ** を確認。
    *   **「1号艇が単勝一番人気でない（2号艇以降が人気）」** の場合のみ通知対象とする。
    *   ※1号艇が1番人気の（堅そうな）レースは通知されません。
4.  **Notify**: Discordに「波乱レース」として通知が届く。
